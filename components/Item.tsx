...
export const Item: NextPage<ItemProps> = ({task, selecionarTarefa}) => {
    ...
    const getDateText = (finishDate : string | undefined, finishPrevisionDate : string) => {
        if(finishDate){
            return `Concluído em: ${moment(finishDate.replace(/-/g, '\/').replace(/T.+/, '')).format('DD/MM/yyyy')}`;
        }

        return `Conclusão em: ${moment(finishPrevisionDate.replace(/-/g, '\/').replace(/T.+/, '')).format('DD/MM/yyyy')}`;
    }
